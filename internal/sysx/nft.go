...
// ifaceSet renders an anonymous set of interface names, and it is the one
// place that does: every `{ "wg-main", "wg-lte1" }` on either host comes from
// here, so the dedupe (a repeated element has nft reject the whole table) and
// the sanitising (nftSafe) cannot drift between the three tables that carry
// one. Written out three times, the next hardening pass landed in whichever
